...
Object.defineProperty(window, "use", {
  get: () => {
    ...
    return (ptr, mapping, ...rest) => {
      ...
      assert(isDLPtr$1(ptr), "a value was passed into use() that was not part of a stateful context");
      __use_trap = false;
...
Object.assign(window, { isDLPtr: isDLPtr$1, h: h$1, stateful: stateful$1, handle: handle$1, $if, Fragment });
...
function isDLPtr$1(arr) {
  return isobj(arr) && TARGET in arr
}

function $if(condition, then, otherwise) {
  otherwise ??= document$1.createTextNode("");
  if (!isDLPtr$1(condition)) return condition ? then : otherwise;

  return { [IF]: condition, then, otherwise };
}

// This lets you subscribe to a stateful object
function handle$1(ptr, callback) {
  assert(isDLPtr$1(ptr), "handle() requires a stateful object");
...
function h$1(type, props, ...children) {
  ...
  if (typeof type == "function") {
    ...
    for (let name in props) {
      ...
      if (name.startsWith("bind:")) {
        assert(isDLPtr$1(ptr), "bind: requires a reference pointer from use");
        ...
        if (propname == "this") {
          ...
          handle$1(use(newthis[propname]), value => {
            if (isRecursive) {
              isRecursive = false;
              return;
            }
  ...
  let elm = xmlns ? document$1.createElementNS(xmlns, type) : document$1.createElement(type);


  for (let child of children) {
    let cond = child && !isDLPtr$1(child) && child[IF];
  ...
  for (let name in props) {
    ...
    if (name.startsWith("bind:")) {
      ...
      if (propname == "this") {
        ...
        handle$1(ptr, value => elm.value = value);
        elm.addEventListener("change", () => set(elm.value));
      } else if (propname == "checked") {
        handle$1(ptr, value => elm.checked = value);
    ...
    if (name == "style" && isobj(ptr)) {
      for (let key in ptr) {
        let prop = ptr[key];
        if (isDLPtr$1(prop)) {
          handle$1(prop, value => elm.style[key] = value);
        } else {
          elm.style[key] = prop;
        }
      }
  ...
  useProp("class", classlist => {
    ...
    if (isDLPtr$1(classlist)) {
      handle$1(classlist, classname => elm.setAttribute("class", classname));
      return;
    }
